...
def value(s as string) as long:
	return "0123456789TJQKA".IndexOf(s[0:1])
	
def are_same_suit(cards as string*):
	return cards.Select({x|x[1]}).Distinct().Count() == 1

def total_value(hand as string*):
	answer = 0
	for i in hand.Select(value).OrderByDescending({x|(x)}):
		answer = answer * 20L + i
	return answer

	
def is_royal_flush(hand as string*):
	a = hand.Select({x|x[0:1]}).Intersect(('T','J','Q','K','A')).Count() == 5
	if a and are_same_suit(hand):
		return 1L
	return 0L
...
def group(hand as string*):
	return hand.GroupBy(value).OrderByDescending({x|x.Count()}).ThenByDescending({x|x.Key})	
...
assert are_same_suit(('4D', '5D')) == true
assert is_royal_flush(('TD', 'JD', 'QD', 'KD', 'AD')) == 1
assert is_flush(('8D', 'TD', 'JD', 'QD', 'KD')) / 1e8 == 1
assert is_straight(('9H', 'TD', 'JD', 'QD', 'KD')) / 1e8 == 1
assert is_straight_flush(('9D', 'TD', 'JD', 'QD', 'KD')) / 1e8 == 1
assert is_four_of_a_kind(('9D', '9H', '9S', '9C', 'KD')) / 1e8 == 9
assert is_three_of_a_kind(('9D', '9H', '9S', '8C', 'KD')) / 1e8 == 9
assert is_full_house(('9D', '9H', '9S', '8C', '8D')) / 1e8 == 908
assert is_two_pairs(('9D', '9H', 'TS', '8C', '8D')) / 1e8 == 908
assert is_one_pair(('9D', '9H', 'TS', '8C', '7D')) / 1e8 == 9
assert total_value(('9D', '9H', 'TS', '8C', '7D')) == ((((10*20+9)*20+9)*20+8)*20+7)


answer = hands.Where(player1_win).Count()
print answer
assert answer == 376