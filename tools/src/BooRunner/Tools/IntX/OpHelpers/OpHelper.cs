...
namespace BooRunner.Tools
{
	...
	static internal class OpHelper
	{
		...
		static public BigInteger Sub(BigInteger int1, BigInteger int2)
		{
			...
			if (compareResult == 0) return new BigInteger(); // integers are equal
			if (compareResult < 0)
			{
				smallerInt = int1;
				biggerInt = int2;
		...
		static public BigInteger Pow(BigInteger value, uint power, MultiplyMode multiplyMode)
		{
			...
			if (value._length == 0) return new BigInteger();
        ...
        /// <returns>Number in given power.</returns>
        /// <exception cref="ArgumentNullException"><paramref name="value" /> is a null reference.</exception>
        static public BigInteger Pow(BigInteger value, uint power, BigInteger mod, MultiplyMode multiplyMode)
        {
            // Exception
            if (ReferenceEquals(value, null))
            {
            ...
            // Return the number itself from a power of one
            if (power == 1) return new BigInteger(value) % mod;
		...
		static public BigInteger Sh(BigInteger intX, long shift, bool toLeft)
		{
			// Exceptions
			...
			if (intX._length == 0) return new BigInteger();

			// Can't shift on zero value
			if (shift == 0) return new BigInteger(intX);
			...
			if (!toLeft && bitCount >= intXBitCount) return new BigInteger();
			...
			BigInteger newInt = new BigInteger(newLength, intX._negative);
